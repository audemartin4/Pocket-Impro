...
/* Fiches qui se recoupent, repérées à la relecture des lots d'exercices (« Alerte : proche de… »).
   Règle demandée par Aude : le générateur de cours ne doit jamais en enchaîner deux à la suite —
   deux variantes du même jeu coup sur coup, ça fait doublon dans un cours. En revanche, un clic sur
   « Aléatoire » depuis une carte reste libre de proposer la fiche voisine : là, c'est un choix
   explicite de l'utilisateur, pas une suggestion de l'appli.
   Table de titres plutôt que champ de fiche : la relation est symétrique par construction, elle se
   lit d'un coup d'œil, et elle n'oblige pas à migrer les fiches déjà en base. Un titre qui n'existe
   pas (encore) dans la bibliothèque ne gêne pas : la paire ne s'applique simplement pas. Renommer
   une fiche, en revanche, casse le lien en silence. */
const EXERCICES_PROCHES = [
  // Différence : « Attente » est un solo sans parole, tout repose sur la façon d'occuper le temps.
  ["Attente", "La file d'attente"],
  // Différence : le chant se greffe sur une action concrète en cours, au lieu de remplacer un dialogue.
  ["Chanliloquer", "Chanson au quotidien"],
  ["Chanliloquer", "Comédie musicale"],
  // Différence : les zones portent des actions physiques, ni des émotions ni des sols imaginaires.
  ["4 pôles", "Pôles d'émotions"],
  ["4 pôles", "Grille environnement"],
  // Différence : la reconnaissance passe par l'étreinte entière plutôt que par le visage.
  ["Accolade", "Qui es-tu ?"],
  // Différence : on se regroupe par émotion et non par cri convenu, chacun inventant son son.
  ["7 familles", "Les piou-piou"],
  // Différence : les yeux restent ouverts et on se reconnaît en jouant un état, pas en émettant un son.
  ["Âmes-sœurs", "7 familles"],
  ["Âmes-sœurs", "Les piou-piou"],
  // Différence : un solo sans contradicteur, où le crime se révèle par les contradictions du personnage.
  ["Accusation", "Procès délirant"],
  ["Accusation", "Arrestation police"],
  // Différence : c'est le rire du public, et non une proposition de jeu, qui déclenche la reprise.
  ["Déceler le rire", "Remarquer et répéter"],
];
// Index titre normalisé -> titres voisins, construit une fois au chargement du module.
const PROCHES_PAR_TITRE = EXERCICES_PROCHES.reduce((index, [a, b]) => {
  (index[normalize(a)] = index[normalize(a)] || new Set()).add(normalize(b));
  (index[normalize(b)] = index[normalize(b)] || new Set()).add(normalize(a));
  return index;
}, {});
const sontProches = (a, b) =>
  !!a && !!b && !!PROCHES_PAR_TITRE[normalize(a.title)]?.has(normalize(b.title));
// Écarte du tirage les fiches qui recoupent celle qu'on vient de retenir — sauf s'il ne reste
// qu'elles : mieux vaut un enchaînement discutable qu'un créneau vide.
const sansLesProchesDe = (pool, precedente) => {
  if (!precedente) return pool;
...
const EXERCICES_ECHAUFFEMENT = [
  ...
  { title: "Killer / Bang / Cowboy", groupe: "Groupe, prénoms et confiance", summary: "En cercle le prof annonce le prénom d'un participant. Cette personne se baisse et les 2 autres à ses côtés se tirent dessus en disant BANG. Le plus lent est éliminé. Si une mort est jouée de façon convaincante on peut ressusciter. Variante « cowboy au centre » : au lieu que l'intervenant annonce le prénom, un joueur se place au centre, dégaine un revolver imaginaire et vise quelqu'un en criant son prénom, ou « pan ! » s'il ne le connaît pas ; il peut aussi lancer une dynamite imaginaire en criant « boom ! », et tout le monde doit alors se baisser en même temps, une seule dynamite par tour.", level: "Débutant", objectives: ["Prénom", "Cohésion", "Cercle"], players: 0, duration: 8, energy: "Forte", material: "Aucun", format: "En cercle" },
  ...
  { title: "Chouba / Ya", groupe: "Cercle", summary: "Jeu d'énergie avec 3 mouvements de base (Chouba/Ya, Bloc, Wiz), auquel on peut ajouter librement d'autres mouvements (expressions, ascenseur, boule de bowling, rio, anniversaire...). Variante pour les groupes déjà très à l'aise : on remplace le « Ya » par le premier mot qui vient à l'esprit en entendant le mot précédent, ce qui superpose le jeu d'énergie et l'association d'idées.", level: "Débutant", objectives: ["Cercle", "Jeu", "Mime", "Cohésion", "Concentration", "Écoute", "Énergie"], players: 0, duration: 7, energy: "Forte", material: "Aucun", format: "En cercle" },
  ...
  { title: "Association d'idées", groupe: "Cercle", summary: "On associe des idées en chaîne les unes après les autres, puis on retrouve la séquence complète, ou on alterne avec un temps limité une fois sur deux. Trois variantes de contrainte, à ajouter une fois la mécanique de base acquise : chaque mot doit être dit avec un accent différent ; chaque mot doit commencer par une seule et même lettre imposée à tout le groupe ; chaque mot doit commencer par la lettre qui suit, dans l'ordre alphabétique, celle du mot précédent.", level: "Débutant", objectives: ["Cercle", "Imagination", "Lâcher-prise"], players: 0, duration: 5, energy: "Modérée", material: "Aucun", format: "En cercle", warmup: false },
  ...
  { title: "Sur consignes", groupe: "Marche", summary: "En déambulation, le groupe réagit à des consignes variées : chiffres, vitesses, terrains imaginaires (boue, neige, verglas...), émotions, ennemi/protecteur, triangle secret, idolâtrer... Variante : à chaque claquement de mains, chacun adopte immédiatement un nouveau mode de déplacement et le garde jusqu'au signal suivant : à quatre pattes, en rampant, bras en l'air, en boitant, en sautillant, en glissant.", level: "Débutant", objectives: ["Déambulation", "Concentration", "Cohésion", "Imagination", "Émotions", "Personnages", "Lieux", "Mime"], players: 0, duration: 10, energy: "Modérée", material: "Aucun", format: "Déambulation", warmup: false },
...
const EXERCICES_PRE_IMPRO = [
  ...
  { title: "Bataille au ralenti", summary: "Comme la course au ralenti, mais sous forme de bataille à l'arme blanche entre 2 clans, joué au ralenti et de façon très expressive. Variante « saloon » : au lieu de deux clans qui s'affrontent, tout le monde se répartit dans l'espace comme au cœur d'une rixe de saloon et déclenche une bagarre générale de chacun contre chacun, où l'on peut aussi éviter un projectile imaginaire, trébucher sur une chaise ou s'effondrer théâtralement.", level: "Débutant", objectives: ["Imagination", "Physique", "Mime", "Musique"], players: 0, duration: 4, energy: "Modérée", material: "Musique (type \"Les Chariots de feu\")", format: "En groupe simultané" },
...
const EXERCICES_IMPRO = [
  ...
  { title: "Les spectateurs", groupe: "Acceptation et écoute", summary: "Le groupe se place en lignes, tourné dans la même direction comme un vrai public, pour \"regarder\" ensemble un événement imaginaire annoncé par l'animateur (décollage de fusée, match de tennis...). Variantes : les joueurs sont assis en ligne sur des chaises comme au cinéma et regardent un film dont l'intervenant donne le ton scène après scène, qu'ils commentent à voix haute ou pas, en ressentant tous la même émotion au même moment (ou, plus vague encore, \"il se passe quelque chose ici\").", level: "Débutant", objectives: ["Écoute", "Acceptation", "Émotions"], players: 0, duration: 5, energy: "Modérée", material: "Aucun", format: "En groupe simultané" },
  ...
  { title: "Demander une faveur", groupe: "Gérer l'instable et l'obstacle", summary: "A demande une faveur à B qui résiste ; A doit argumenter et expliquer la raison de sa demande. Variante : au lieu d'une faveur, A cherche à obtenir de B qu'il fasse ou dise quelque chose qu'il ne veut pas faire, en insistant, en argumentant ou en mettant la pression, pendant que B résiste, hésite, contourne, puis cède progressivement pour faire avancer la scène.", level: "Confirmé", objectives: ["Imprévu", "Acceptation", "Écoute", "Jeu de la scène"], players: 2, duration: 10, energy: "Faible", material: "Aucun", format: "Tour à tour avec spectateur" },
...
/* Lot de 74 exercices fourni par Aude le 2026-09-03 (PDF « Pocket Impro - Nouveaux
   exercices (1) »), déjà rédigé au vocabulaire de l'appli : sections, niveaux, formats, familles et
   tags sont repris tels quels. `publics` n'est encore lu par aucun écran : il attend le « choix par
   tranche d'âge » annoncé sur l'accueil, et se perdrait si on ne l'enregistrait pas maintenant.
   Huit de ces fiches recoupent une fiche voisine : le lien est déclaré dans EXERCICES_PROCHES, pas
   ici, pour rester lisible et ne pas dépendre d'une migration. */
const EXERCICES_NOUVEAUX_2026 = [
  { title: "Bruitage d'objets", groupe: "Cercle", summary: "Chacun son tour, les joueurs imitent uniquement avec la voix (pas de mime) le bruit d'au moins cinq objets du quotidien en mouvement, sans jamais répéter un objet déjà proposé. Selon la variante choisie, le public devine l'objet imité, ou le joueur l'annonce lui-même avant de laisser sa place au suivant.", level: "Débutant", objectives: ["Cohésion", "Créativité", "Mémoire", "Objet"], players: 0, duration: 15, energy: "Modérée", material: "Aucun", format: "En cercle", phase: "Échauffement", warmup: true, publics: ["Enfants", "Ados", "Senior", "Adultes"] },
  { title: "Émotions au bout des pieds", groupe: "Incarner les émotions", summary: "Un joueur entre en scène en mettant ses pieds bien en vue du public, sans utiliser la parole. Il leur donne une vie propre, comme deux marionnettes, et leur fait traverser différentes émotions et situations à travers leurs déplacements, leurs contacts et leurs rythmes, en gardant le reste du corps discret.", level: "Avancé", objectives: ["Corps", "Créativité", "Émotions", "Concentration"], players: 0, duration: 15, energy: "Faible", material: "Aucun", format: "Tour à tour avec spectateur", phase: "Impro", publics: ["Adultes"] },
  { title: "Visage émotions", groupe: "Incarner les émotions", summary: "Le joueur a la tête baissée. L'intervenant choisit une émotion. Le joueur relève lentement la tête, sur laquelle se lit cette émotion. S'il craque, il rebaisse la tête aussitôt et recommence.", level: "Confirmé", objectives: ["Émotions", "Regard", "Concentration"], players: 0, duration: 10, energy: "Faible", material: "Aucun", format: "Tour à tour avec spectateur", phase: "Pré-impro", warmup: true, publics: ["Ados", "Adultes"] },
  { title: "Une phrase 4 chaises", groupe: "Incarner les émotions", summary: "Quatre chaises sont disposées en ligne ou en carré, chacune associée à une émotion. Les joueurs reçoivent tous la même phrase, simple et neutre. Le premier s'assoit sur la première chaise et dit la phrase avec l'émotion correspondante, puis passe à la deuxième, et ainsi de suite. Quand il a terminé, il libère sa place et un nouveau joueur entre : le groupe avance par rotation.", level: "Confirmé", objectives: ["Émotions", "Voix", "Rythme", "Écoute"], players: 0, duration: 15, energy: "Modérée", material: "4 chaises", format: "Tour à tour avec spectateur", phase: "Pré-impro", publics: ["Ados", "Adultes", "Senior"] },
  { title: "Docteur inquiétant", groupe: "Comédie du jeu de la scène", summary: "Un personnage inquiétant remplace le médecin derrière son bureau ou le kinésithérapeute à côté de sa table de massage, et reçoit un patient venu consulter pour un problème inventé. Il donne des ordres, pose des questions, manipule le patient et propose des soins improbables. Trois variantes. Simple : la scène se joue à deux. Avec assistant : un troisième joueur comprend mal les consignes, se trompe sans cesse et complique tout. Avec le public : le patient est une personne invitée à monter sur scène.", level: "Avancé", objectives: ["Personnages", "Statut", "Enjeux", "Créativité"], players: 0, duration: 15, energy: "Modérée", material: "Une table ou un bureau", format: "Tour à tour avec spectateur", phase: "Impro", publics: ["Ados", "Adultes", "Senior"] },
  { title: "Tics et tocs", groupe: "Approfondir le personnage", summary: "Les joueurs entrent dans la peau de personnages obsessionnels. Chacun reçoit ou invente un ou plusieurs tics et troubles compulsifs irrépressibles qu'il reproduit en permanence : gestes répétitifs, vérifications, comptages, manies, rituels, paroles automatiques. Ils construisent une situation commune sans jamais abandonner leurs obsessions.", level: "Confirmé", objectives: ["Personnages", "Corps", "Physique", "Concentration"], players: 0, duration: 15, energy: "Modérée", material: "Aucun", format: "En groupe simultané", phase: "Impro", publics: ["Ados", "Adultes"] },
  { title: "Attente", groupe: "Jeu du corps", summary: "Un personnage entre seul et se place dans une situation d'attente : passer à son tour pour un rendez-vous, un examen, une audition. Il ne parle pas. Il occupe le temps en trouvant une multitude d'actions, de gestes et de micro-situations pour faire exister l'ennui, l'impatience ou la nervosité, uniquement par le corps, le regard et le rapport à l'espace.", level: "Avancé", objectives: ["Corps", "Silence", "Rythme", "Concentration"], players: 0, duration: 15, energy: "Faible", material: "Aucun", format: "Tour à tour avec spectateur", phase: "Impro", publics: ["Ados", "Adultes", "Senior"] },
  { title: "Traversée impossible", groupe: "Gérer l'instable et l'obstacle", summary: "Le joueur doit traverser la scène en ligne droite, d'un point A clairement désigné vers un point B situé en face. L'intention de départ est parfaitement claire. Mais à mesure qu'il avance, tout devient obstacle : une chaussure se coince ou se déchausse, un objet imaginaire provoque une glissade, un fil invisible le tire en arrière, une idée soudaine le détourne.", level: "Avancé", objectives: ["Corps", "Imagination", "Rythme", "Physique"], players: 0, duration: 15, energy: "Modérée", material: "Aucun", format: "Tour à tour avec spectateur", phase: "Impro", publics: ["Ados", "Adultes"] },
  { title: "Playback", groupe: "Jouer avec la musique", summary: "Chacun choisit une chanson qu'il connaît et l'interprète en playback, en synchronisant précisément ses lèvres avec le texte et en assumant pleinement la mise en scène, l'énergie et la présence face au public. Deux variantes. Improvisée : la chorégraphie se cherche en direct. Préparée : chacun compose d'abord une chorégraphie, la mémorise, puis présente l'ensemble comme un vrai numéro.", level: "Avancé", objectives: ["Musique", "Corps", "Engagement", "Rythme"], players: 0, duration: 15, energy: "Forte", material: "Une musique choisie par chaque joueur", format: "Tour à tour avec spectateur", phase: "Impro", publics: ["Ados", "Adultes"] },
  { title: "Chanliloquer", groupe: "Jouer avec la musique", summary: "Un personnage entre avec une action simple à accomplir : balayer, ranger, réparer, ou toute autre tâche du quotidien. Il l'effectue d'abord de manière concrète et crédible. Progressivement, sans jamais interrompre son action, il se met à improviser une chanson en lien avec ce qu'il fait, ce qu'il vit ou ce qu'il observe. Le chant naît de l'action et s'y intègre jusqu'à constituer une scène entièrement chantée.", level: "Avancé", objectives: ["Musique", "Voix", "Créativité", "Corps"], players: 0, duration: 15, energy: "Modérée", material: "Aucun", format: "Tour à tour avec spectateur", phase: "Impro", publics: ["Ados", "Adultes"] },
  { title: "Vidéo clip", groupe: "Jouer avec la musique", summary: "Une musique est diffusée. Un joueur tient une caméra, vraie ou fausse, et se déplace librement comme s'il réalisait un tournage. Trois autres évoluent simultanément devant cet objectif imaginaire, en tenant compte en permanence de la présence du cadreur, de ses déplacements et de l'orientation supposée de la caméra. C'est le cadreur qui choisit ce qu'il filme.", level: "Avancé", objectives: ["Musique", "Regard", "Corps", "Cohésion"], players: 0, duration: 15, energy: "Forte", material: "Musique et une caméra vraie ou fausse", format: "En groupe simultané", phase: "Impro", publics: ["Ados", "Adultes"] },
  { title: "4 pôles", groupe: "Physique", summary: "L'intervenant définit quatre espaces. Dans le premier les joueurs dansent, dans le deuxième ils pratiquent la danse contact, dans le troisième ils se regardent profondément dans les yeux, dans le dernier ils font semblant de retirer de la poussière partout sur leur corps. Chacun doit vivre pleinement ce qui est demandé dans chaque espace et passer de l'un à l'autre très rapidement. On peut ajouter des zones et d'autres actions.", level: "Débutant", objectives: ["Corps", "Contact", "Regard", "Énergie"], players: 0, duration: 10, energy: "Modérée", material: "Aucun", format: "En groupe simultané", phase: "Échauffement", warmup: true, publics: ["Enfants", "Ados", "Adultes"] },
  { title: "Strip-tease mimé", groupe: "Jeu du corps", summary: "Un personnage entre en scène et mime un strip-tease sans jamais retirer le moindre vêtement. Il simule chaque étape du déshabillage avec précision, comme si les habits existaient réellement, en jouant les textures, les résistances, les gestes et les pauses. Le corps reste entièrement habillé : tout repose sur la suggestion, le regard, le rythme, la respiration et l'adresse au public.", level: "Avancé", objectives: ["Mime", "Corps", "Regard", "Rythme"], players: 0, duration: 15, energy: "Modérée", material: "Aucun", format: "Tour à tour avec spectateur", phase: "Impro", publics: ["Adultes"] },
  { title: "Accolade", groupe: "Groupe, prénoms et confiance", summary: "Les joueurs se placent en cercle. L'un d'eux ferme les yeux et reste immobile au centre. Un autre sort du cercle, s'approche silencieusement et le prend dans ses bras sans parler. Celui qui a les yeux fermés doit reconnaître la personne uniquement par le contact et annoncer son prénom. Les rôles changent pour que chacun vive les deux positions. À proposer après un temps de rencontre préalable entre les participants.", level: "Débutant", objectives: ["Contact", "Confiance", "Prénom", "Complicité"], players: 0, duration: 10, energy: "Faible", material: "Aucun", format: "En cercle", phase: "Échauffement", warmup: true, publics: ["Enfants", "Ados", "Adultes", "Senior"] },
  { title: "7 familles", groupe: "Groupe, prénoms et confiance", summary: "L'intervenant attribue secrètement une émotion à chaque joueur. Tous se répartissent dans l'espace, ferment les yeux au signal et se déplacent lentement en silence pour chercher les autres. Quand deux joueurs se rencontrent, aucun mot n'est autorisé : chacun émet uniquement un son inspiré de l'émotion qu'il a reçue. Les familles se reconstituent ainsi, à l'oreille.", level: "Avancé", objectives: ["Écoute", "Émotions", "Voix", "Cohésion"], players: 0, duration: 5, energy: "Modérée", material: "Aucun", format: "En groupe simultané", phase: "Échauffement", warmup: true, publics: ["Enfants", "Ados", "Adultes", "Senior"] },
  { title: "Meilleurs titres", groupe: "Cercle", summary: "Un joueur choisit le titre d'une œuvre connue, chanson, film, opéra, série ou spectacle, et l'annonce au groupe. Chacun doit alors proposer à voix haute un nouveau titre de remplacement pour cette œuvre. Le jeu se poursuit jusqu'à ce que plusieurs alternatives aient été proposées, puis un autre joueur choisit un nouveau titre.", level: "Confirmé", objectives: ["Créativité", "Imagination", "Jeu", "Spontanéité"], players: 0, duration: 15, energy: "Modérée", material: "Aucun", format: "En cercle", phase: "Pré-impro", warmup: true, publics: ["Ados", "Adultes", "Senior"] },
  { title: "Le radar", groupe: "Le contact", summary: "Un joueur est assis sur une chaise face au public, les yeux fermés. Un autre s'approche silencieusement et vient se placer à sa gauche ou à sa droite, sans se faire repérer. Un troisième demande alors au joueur assis de deviner de quel côté se trouve la personne. Il pointe la direction du doigt, puis on vérifie. Les rôles tournent.", level: "Débutant", objectives: ["Écoute", "Concentration", "Sensations", "Jeu"], players: 0, duration: 15, energy: "Faible", material: "Une chaise", format: "Tour à tour avec spectateur", phase: "Échauffement", warmup: true, publics: ["Ados", "Adultes", "Senior"] },
  { title: "Âmes-sœurs", groupe: "Groupe, prénoms et confiance", summary: "L'animateur prépare des papiers sur lesquels chaque indication est écrite deux fois, de sorte que chaque situation existe en double. Chacun pioche un papier sans le montrer, puis joue l'état ou l'attitude qu'il a reçu : être très heureux, avoir perdu quelque chose, être en colère, écouter de la musique, avoir mal à un pied. L'objectif est de retrouver la personne qui joue exactement la même chose.", level: "Débutant", objectives: ["Observation", "Cohésion", "Jeu", "Émotions"], players: 0, duration: 15, energy: "Modérée", material: "Des papiers préparés à l'avance", format: "En groupe simultané", phase: "Échauffement", warmup: true, publics: ["Enfants", "Ados", "Adultes"] },
  { title: "Cadeau mystère", groupe: "Incarner les émotions", summary: "L'intervenant remet un paquet soigneusement emballé à un joueur, qui le reçoit comme un présent unique et précieux. Il observe l'emballage, le manipule, écoute les sons qu'il produit, en explore les moindres détails avant d'oser l'ouvrir. Le contenu peut être quelconque ou surprenant, un rouleau de papier toilette, une gamelle, un vieux jouet, et le joueur réagit librement à cette découverte.", level: "Débutant", objectives: ["Objet", "Émotions", "Sensations", "Regard"], players: 0, duration: 15, energy: "Faible", material: "Un paquet emballé et son contenu", format: "Tour à tour avec spectateur", phase: "Impro", publics: ["Enfants", "Ados", "Adultes", "Senior"] },
  { title: "Le monstre", groupe: "Comédie du jeu de la scène", summary: "Un joueur incarne un monstre accusé d'avoir détruit un lieu, un objet ou une institution : une ville miniature, un musée, une école, un monument. La nature de la destruction est décidée avant l'entrée en scène. Le monstre se place face au groupe comme lors d'une conférence de presse officielle, et les autres jouent les journalistes qui l'interrogent.", level: "Avancé", objectives: ["Personnages", "Argumentation", "Créativité", "Enjeux"], players: 0, duration: 15, energy: "Modérée", material: "Aucun", format: "Tour à tour avec spectateur", phase: "Impro", publics: ["Ados", "Adultes"] },
  { title: "La star ne vient pas", groupe: "Comédie du jeu de la scène", summary: "Le personnage annonce l'arrivée d'une star. On lui apprend alors qu'elle est bloquée et ne viendra pas tout de suite. Il doit meubler face au public, seul, aussi longtemps qu'il le faut. D'autres personnages peuvent intervenir ponctuellement pour relancer l'enjeu ou aggraver la situation.", level: "Avancé", objectives: ["Enjeux", "Voix", "Imagination", "Relation"], players: 0, duration: 15, energy: "Forte", material: "Aucun", format: "Tour à tour avec spectateur", phase: "Impro", publics: ["Ados", "Adultes"] },
  { title: "Accusation", groupe: "Approfondir le personnage", summary: "Le joueur entre seul pour plaider son innocence devant le public, comme s'il comparaissait face à un tribunal invisible. Il affirme d'abord n'avoir rien fait, être victime d'une erreur ou d'un malentendu. Mais au fil de son plaidoyer, des détails surgissent, des contradictions apparaissent, et le spectateur comprend peu à peu la nature exacte du crime qu'il a commis.", level: "Avancé", objectives: ["Argumentation", "Personnages", "Enjeux", "Narration"], players: 0, duration: 15, energy: "Modérée", material: "Aucun", format: "Tour à tour avec spectateur", phase: "Impro", publics: ["Ados", "Adultes"] },
  { title: "Fin du monde", groupe: "Posture confiante", summary: "Le personnage entre pour annoncer au public que la fin du monde est imminente, ou déjà en cours. Il s'adresse directement aux spectateurs comme s'il portait une information capitale, urgente et incontestable. Mais il constate l'absence totale de réaction. Cette indifférence l'étonne, le trouble, le déstabilise, et il ajuste alors son discours, son ton et son énergie pour tenter d'obtenir enfin quelque chose.", level: "Avancé", objectives: ["Voix", "Regard", "Engagement", "Enjeux"], players: 0, duration: 15, energy: "Forte", material: "Aucun", format: "Tour à tour avec spectateur", phase: "Impro", publics: ["Ados", "Adultes"] },
  { title: "Applaudissements", groupe: "Posture confiante", summary: "Le personnage entre seul, effectue un tour complet du plateau à un rythme choisi, puis vient se placer à l'avant-scène face au public. L'entrée doit être clairement visible et assumée, sans parole. Le public applaudit dès qu'il s'installe. Le personnage accueille ces applaudissements, les laisse exister, prend le temps de les recevoir et d'y réagir sans les couper.", level: "Avancé", objectives: ["Posture", "Regard", "Silence", "Engagement"], players: 0, duration: 15, energy: "Faible", material: "Aucun", format: "Tour à tour avec spectateur", phase: "Pré-impro", stageWarmup: true, publics: ["Ados", "Adultes"] },
  { title: "Déceler le rire", groupe: "Comédie du jeu de la scène", summary: "Les joueurs passent un par un ou en duo, sur un thème annoncé avant chaque passage. Une fois la scène lancée, ils jouent normalement mais restent attentifs au public. À chaque rire entendu, discret ou franc, ils doivent en tenir compte immédiatement : reprendre ce qui l'a déclenché, l'amplifier, le relancer, plutôt que de poursuivre comme si de rien n'était.", level: "Avancé", objectives: ["Écoute", "Jeu de la scène", "Regard", "Rythme"], players: 0, duration: 15, energy: "Modérée", material: "Aucun", format: "Tour à tour avec spectateur", phase: "Impro", publics: ["Ados", "Adultes"] },
  { title: "Sales et méchants", groupe: "Approfondir le personnage", summary: "Les joueurs incarnent des personnages volontairement épouvantables, construits à partir de traits excessifs : apparence négligée ou repoussante, comportement vulgaire, attitudes sales, langage grossier. Ils développent une situation commune et interagissent en restant fidèles à cette identité outrancière, sans jamais l'adoucir.", level: "Confirmé", objectives: ["Personnages", "Corps", "Voix", "Lâcher-prise"], players: 0, duration: 15, energy: "Forte", material: "Aucun", format: "En groupe simultané", phase: "Impro", publics: ["Ados", "Adultes"] },
  { title: "Personnage robot", groupe: "Jeu du corps", summary: "Le joueur entre en incarnant un robot, avec une identité singulière définie par une fonction précise et une personnalité propre. Le corps adopte une organisation mécanique : déplacements segmentés, gestes précis, rythmes répétitifs ou programmés. Si la voix est utilisée, elle suit la même logique de débit, de modulation et de rupture.", level: "Débutant", objectives: ["Corps", "Personnages", "Rythme", "Voix"], players: 0, duration: 15, energy: "Modérée", material: "Aucun", format: "Tour à tour avec spectateur", phase: "Pré-impro", publics: ["Ados", "Adultes"] },
  { title: "Ma vraie enfance", groupe: "Approfondir le personnage", summary: "Chacun intervient à son tour, seul face au groupe, pour décrire sa propre enfance en s'appuyant uniquement sur des souvenirs réels, sans rien inventer ni transformer volontairement les faits. Il raconte librement, à la première personne, des moments, des situations, des lieux ou des images marquantes.", level: "Avancé", objectives: ["Narration", "Émotions", "Voix", "Engagement"], players: 0, duration: 15, energy: "Faible", material: "Aucun", format: "Tour à tour avec spectateur", phase: "Impro", publics: ["Ados", "Adultes"] },
  { title: "Kamasutra", groupe: "Jeu du corps", summary: "Chacun vient sur scène l'un après l'autre. Il annonce le nom d'une position sexuelle, la décrit de manière simple et factuelle, puis la mime de façon stylisée et symbolique, sans jamais chercher le réalisme ni la provocation. La présentation se fait dans une neutralité totale, comme une démonstration technique ou pédagogique adressée au public.", level: "Avancé", objectives: ["Corps", "Mime", "Posture", "Lâcher-prise"], players: 0, duration: 15, energy: "Faible", material: "Aucun", format: "Tour à tour avec spectateur", phase: "Pré-impro", publics: ["Adultes"] },
  { title: "Bagarre fantôme", groupe: "Jeu du corps", summary: "Le joueur entre seul et se bat contre un adversaire invisible que lui seul peut voir. Il enchaîne coups, esquives, chutes et projections en donnant l'impression qu'un partenaire est réellement présent. Les réactions doivent être précises et cohérentes, comme si chaque action du fantôme avait un impact réel sur son corps.", level: "Avancé", objectives: ["Corps", "Mime", "Physique", "Engagement"], players: 0, duration: 15, energy: "Forte", material: "Aucun", format: "Tour à tour avec spectateur", phase: "Impro", publics: ["Ados", "Adultes"] },
  { title: "Le corps qui bug", groupe: "Jeu du corps", summary: "Le joueur se met en mouvement dans l'espace et choisit une partie précise de son corps qui va bugger, une main, la tête, une jambe. Cette partie répète en boucle un mouvement court et identique, de manière continue. Le reste du corps continue à fonctionner normalement, marcher, se déplacer, agir, interagir, sans jamais que la boucle s'interrompe.", level: "Confirmé", objectives: ["Corps", "Rythme", "Concentration", "Physique"], players: 0, duration: 15, energy: "Modérée", material: "Aucun", format: "Déambulation", phase: "Échauffement", warmup: true, publics: ["Ados", "Adultes"] },
  { title: "Étymologie", groupe: "Vocal", summary: "L'intervenant donne un mot au groupe, choisi librement et annoncé clairement. Un joueur prend la parole et invente entièrement son étymologie. Il développe une origine imaginaire en s'appuyant sur des associations sonores, des racines supposées, des images, des anecdotes ou des références fictives. Le discours peut se faire savant, poétique ou franchement délirant.", level: "Confirmé", objectives: ["Voix", "Imagination", "Créativité", "Narration"], players: 0, duration: 5, energy: "Modérée", material: "Aucun", format: "En cercle", phase: "Pré-impro", warmup: true, publics: ["Ados", "Adultes"] },
  { title: "Bataille de poules", groupe: "Physique", summary: "Dans un espace délimité au sol dégagé, tous se mettent en position accroupie, pieds bien ancrés et mains posées sur les genoux. Au signal, chacun tente de faire perdre l'équilibre aux autres en les bousculant légèrement avec les épaules, sans jamais utiliser les mains. Les déplacements restent courts et contrôlés. Qui pose une main au sol est éliminé.", level: "Débutant", objectives: ["Corps", "Physique", "Jeu", "Énergie"], players: 0, duration: 5, energy: "Forte", material: "Aucun", format: "En groupe simultané", phase: "Échauffement", warmup: true, publics: ["Enfants", "Ados", "Adultes"] },
  { title: "Fuzzy Ducky", groupe: "Cercle", summary: "En cercle, un joueur dit « fuzzy » en regardant son voisin. Celui-ci répète « fuzzy » et la parole continue de proche en proche dans le même sens. À tout moment, quelqu'un peut dire « ducky » au lieu de « fuzzy » : le sens de circulation s'inverse alors immédiatement. Qui se trompe ou hésite sort du cercle.", level: "Débutant", objectives: ["Cercle", "Concentration", "Voix", "Jeu"], players: 0, duration: 5, energy: "Modérée", material: "Aucun", format: "En cercle", phase: "Échauffement", warmup: true, publics: ["Enfants", "Ados", "Adultes", "Senior"] },
  { title: "Le chat fait rire", groupe: "Déconnexion", summary: "Un joueur est désigné comme le chat, les autres s'assoient en cercle autour de lui. Le chat se déplace à quatre pattes d'un joueur à l'autre en miaulant. Quand il s'arrête devant quelqu'un, celui-ci doit le caresser en le regardant dans les yeux, sans rire, et lui parler calmement, par exemple « pauvre petit chat ». Le chat fait tout pour le faire craquer.", level: "Débutant", objectives: ["Jeu", "Regard", "Lâcher-prise", "Cercle"], players: 0, duration: 15, energy: "Modérée", material: "Aucun", format: "En cercle", phase: "Échauffement", warmup: true, publics: ["Enfants", "Ados", "Adultes"] },
  { title: "Statues au signal", groupe: "Physique", summary: "Les joueurs se déplacent librement. Au claquement de mains de l'intervenant, tout le monde se fige instantanément. Deux variantes. Photo ! : tous se tournent vers le public comme s'ils posaient pour une photographie de groupe, chacun cherchant à être beau, photogénique et bien placé dans le cadre imaginaire. Statues : chacun se fige exactement là où il en est, sans corriger sa position ; l'intervenant peut imposer un thème, mer, guerre, fête, pour orienter les postures.", level: "Débutant", objectives: ["Corps", "Concentration", "Jeu", "Cohésion"], players: 0, duration: 5, energy: "Forte", material: "Aucun", format: "En groupe simultané", phase: "Échauffement", warmup: true, publics: ["Enfants", "Ados", "Adultes", "Senior"] },
  { title: "Les deux bords", groupe: "Physique", summary: "L'intervenant trace une ligne droite au sol, bien visible. Les joueurs se placent en file indienne face à cette ligne. Il choisit deux mots distincts, l'un associé au côté gauche, l'autre au côté droit. À chaque annonce d'un mot, tous doivent immédiatement franchir la ligne pour se retrouver du bon côté. Le rythme s'accélère et de nouveaux couples de mots s'ajoutent.", level: "Débutant", objectives: ["Concentration", "Jeu", "Corps", "Énergie"], players: 0, duration: 10, energy: "Forte", material: "Du scotch ou une craie", format: "En groupe simultané", phase: "Échauffement", warmup: true, publics: ["Enfants", "Ados", "Adultes"] },
  { title: "Le mur du son", groupe: "Vocal", summary: "Un groupe de joueurs forme un mur humain, ligne continue ou bloc compact, et produit du bruit pour couvrir les voix. De part et d'autre, d'autres joueurs doivent malgré tout se faire comprendre. Deux variantes. Le dialogue : deux joueurs placés de chaque côté du mur tiennent une conversation simple, questions et réponses ou échange d'informations courtes, sans jamais renoncer à se comprendre. Le message : deux équipes s'alignent face à face et un joueur passe derrière la ligne adverse, sans voir les siens. L'intervenant lui murmure une phrase courte, une seule fois, qu'il doit transmettre à sa propre équipe en criant par-dessus l'équipe adverse.", level: "Débutant", objectives: ["Voix", "Écoute", "Concentration", "Cohésion"], players: 0, duration: 15, energy: "Forte", material: "Aucun", format: "En groupe simultané", phase: "Échauffement", warmup: true, publics: ["Enfants", "Ados", "Adultes"] },
  { title: "Voix dans le dos", groupe: "Groupe, prénoms et confiance", summary: "Un joueur se place au centre, dos tourné au reste du groupe. Les autres forment une file derrière lui, hors de son champ de vision. Un par un, ils s'approchent dans son dos et prononcent une phrase courte, choisie à l'avance ou donnée par l'animateur, avec leur voix naturelle. Après chaque prise de parole, celui du centre annonce le prénom de la personne qu'il croit avoir reconnue.", level: "Débutant", objectives: ["Écoute", "Voix", "Prénom", "Concentration"], players: 0, duration: 15, energy: "Faible", material: "Aucun", format: "En groupe simultané", phase: "Échauffement", warmup: true, publics: ["Enfants", "Ados", "Adultes", "Senior"] },
  { title: "Expression fixe", groupe: "Incarner les émotions", summary: "Avant l'entrée en scène, le public ou un joueur hors plateau attribue à chacun une expression faciale précise : joie, tristesse, colère, surprise, dégoût, peur. Les joueurs entrent ensuite et improvisent une situation collective. Ils doivent conserver en permanence l'expression reçue, quoi qu'il arrive dans la scène et quel que soit le sens de leurs répliques.", level: "Avancé", objectives: ["Émotions", "Regard", "Personnages", "Concentration"], players: 0, duration: 15, energy: "Modérée", material: "Aucun", format: "En groupe simultané", phase: "Impro", publics: ["Ados", "Adultes"] },
  { title: "Compagnon invisible", groupe: "Approfondir le personnage", summary: "Le joueur est seul en scène, ou déambule avec les autres, et agit comme s'il était accompagné en permanence par un ami invisible à ses côtés. Il lui fait de la place, adapte ses déplacements, s'adresse à lui et réagit à ses réponses supposées. L'ami n'est jamais montré ni décrit : il n'existe que par les gestes, les regards et les paroles du personnage.", level: "Confirmé", objectives: ["Imagination", "Mime", "Relation", "Regard", "Personnages", "Jouer juste"], players: 0, duration: 10, energy: "Faible", material: "Aucun", format: "Tour à tour avec spectateur", phase: "Pré-impro", publics: ["Ados", "Adultes", "Senior"] },
  { title: "Fleur précieuse", groupe: "Le contact", summary: "Chacun imagine une fleur fragile et très précieuse qu'il tient entre ses mains. Les joueurs se déplacent librement et, quand l'intervenant le demande, l'un s'approche d'un autre et lui tend la fleur en disant : « Tiens, je te la donne ». Un premier tour se fait sans autre consigne, chacun recevant et transmettant la fleur avec délicatesse. Ensuite, l'intervenant annonce une raison que chacun doit incarner en transmettant : pour se faire pardonner, pour l'offrir à son amoureux ou à un parent, pour sauver le monde, pour nourrir une vache affamée, pour déclarer sa flamme, pour un enfant qui n'a jamais vu de fleur, pour un scientifique chargé d'étudier cette espèce inconnue.", level: "Confirmé", objectives: ["Mime", "Objet", "Relation", "Émotions", "Contact", "Déambulation"], players: 0, duration: 15, energy: "Modérée", material: "Aucun", format: "Déambulation", phase: "Pré-impro", stageWarmup: true, publics: ["Enfants", "Ados", "Adultes", "Senior"] },
  { title: "Panier de fruits", groupe: "Cercle", summary: "Les joueurs s'assoient en cercle sur des chaises, avec une chaise en moins que le nombre de participants. Chacun reçoit le nom d'un fruit : pomme, banane, poire, raisin. Un joueur commence au centre, sans chaise, et annonce à voix haute un ou plusieurs fruits. Tous ceux dont le fruit est nommé doivent se lever et changer de place entre eux. Pendant ce déplacement, celui du centre tente de s'asseoir sur une chaise libre. Celui qui se retrouve sans chaise prend la place au centre et annonce à son tour.", level: "Débutant", objectives: ["Cercle", "Jeu", "Écoute", "Mémoire", "Énergie"], players: 0, duration: 5, energy: "Forte", material: "Une chaise par joueur, moins une", format: "En cercle", phase: "Échauffement", warmup: true, publics: ["Enfants", "Adultes", "Senior"] },
  { title: "Bonjour", groupe: "Marche", summary: "Les joueurs se déplacent librement dans l'espace. Quand ils se croisent, ils se disent bonjour. À chaque rencontre, l'intention change : hypocrite, généreuse, enthousiaste, méfiante, glaciale, complice. Le mot reste le même, mais l'intonation, le regard et l'attitude du corps varient à chaque fois. On continue à circuler en multipliant les rencontres et en changeant régulièrement d'intention.", level: "Débutant", objectives: ["Déambulation", "Émotions", "Voix", "Personnages", "Cohésion"], players: 0, duration: 5, energy: "Modérée", material: "Aucun", format: "Déambulation", phase: "Échauffement", warmup: true, stageWarmup: true, publics: ["Enfants", "Ados", "Adultes", "Senior"] },
  { title: "Secret en grommelot", groupe: "Le contact", summary: "Les joueurs marchent librement dans l'espace et se croisent régulièrement. À chaque rencontre, ils s'approchent brièvement et chuchotent à l'oreille de leur partenaire, en grommelot, un événement secret issu de leur vraie vie. Le message est murmuré de façon volontairement incompréhensible, mais en conservant intégralement l'intention, l'émotion et le rythme de la parole. Après l'échange, chacun repart à la rencontre de quelqu'un d'autre.", level: "Avancé", objectives: ["Voix", "Émotions", "Confiance", "Contact", "Déambulation", "Lâcher-prise"], players: 0, duration: 5, energy: "Faible", material: "Aucun", format: "Déambulation", phase: "Pré-impro", warmup: true, stageWarmup: true, publics: ["Adultes"] },
  { title: "La vache sans tache", groupe: "Cercle", summary: "Chacun reçoit un numéro et devient « la vache sans tache n° X ». Le premier appelle : « Je suis la vache sans tache n° 1 et j'appelle la vache sans tache n° 2 ». Le deuxième enchaîne de la même façon, et ainsi de suite. Quand quelqu'un se trompe, il reçoit une tache, une gommette ou un autocollant collé sur son t-shirt, et devient « la vache avec une tache » : il doit désormais s'annoncer ainsi. La vraie difficulté est de se souvenir de son propre nombre de taches, puisqu'on ne voit pas forcément ce qu'on a sur soi.", level: "Débutant", objectives: ["Mémoire", "Concentration", "Écoute", "Jeu", "Cercle"], players: 0, duration: 10, energy: "Modérée", material: "Des gommettes ou autocollants", format: "En cercle", phase: "Échauffement", publics: ["Enfants", "Ados", "Adultes", "Senior"] },
  { title: "Tous à poils !", groupe: "Jeu du corps", summary: "Les joueurs sont répartis dans l'espace, sans disposition imposée. Chacun mime le fait de se déshabiller, en rendant chaque geste lisible et précis, comme s'il retirait de vrais vêtements. Une fois nu, il mime une douche : l'eau, le savon, la température, les sensations, tout doit exister par le mouvement. Quand il se considère entièrement douché, il se rhabille avec de nouveaux vêtements imaginaires, différents des précédents, en marquant clairement les matières, les formes et les usages. Puis il recommence immédiatement le cycle, en se déshabillant à nouveau et en se rhabillant encore autrement. L'exercice tourne en continu.", level: "Débutant", objectives: ["Mime", "Corps", "Imagination", "Objet", "Lâcher-prise"], players: 0, duration: 5, energy: "Modérée", material: "Aucun", format: "Solo simultané", phase: "Échauffement", warmup: true, stageWarmup: true, publics: ["Adultes"] },
  { title: "Le casque invisible", groupe: "Physique", summary: "L'intervenant choisit à l'avance deux genres musicaux bien opposés, par exemple salsa et heavy metal, ou valse et techno, et distribue à chacun un papier portant l'un des deux. Chacun le lit discrètement et le garde pour lui. Consigne : danser comme si on portait un casque audio diffusant sa musique. Aucune musique réelle n'est diffusée, tout se passe en silence. Après un moment de danse libre, chacun doit tenter de retrouver les autres membres de son genre musical, uniquement en observant et en interprétant les mouvements. Le premier groupe réuni au complet a gagné. Plus les genres sont opposés, plus c'est facile.", level: "Débutant", objectives: ["Danse", "Corps", "Observation", "Jeu", "Rythme", "Cohésion"], players: 0, duration: 15, energy: "Forte", material: "Des papiers portant chacun un genre musical", format: "Solo simultané", phase: "Échauffement", publics: ["Enfants", "Ados", "Adultes", "Senior"] },
  { title: "Avion", groupe: "Marche", summary: "Les joueurs se déplacent librement dans l'espace, les bras tendus horizontalement de chaque côté, à hauteur d'épaules. Ils avancent à une allure régulière, sans courir, en gardant une attention constante à leur environnement immédiat. Il faut éviter tout contact, mais sans changements brusques de direction : le déplacement reste continu et fluide, chacun restant attentif à la circulation d'ensemble. Variante en jeu : les deux premiers qui se touchent sont éliminés.", level: "Débutant", objectives: ["Corps", "Écoute", "Déambulation", "Concentration", "Cohésion"], players: 0, duration: 5, energy: "Modérée", material: "Aucun", format: "Déambulation", phase: "Échauffement", warmup: true, stageWarmup: true, publics: ["Enfants", "Ados", "Adultes", "Senior"] },
  { title: "La puce à tous", groupe: "Observation, mimétisme et complicité", summary: "Un personnage commence à se gratter comme s'il avait une puce. Très vite, la puce saute sur quelqu'un d'autre, qui se met à son tour à se gratter. Les joueurs peuvent se la passer volontairement, la perdre, la retrouver, ou tenter de s'en débarrasser. Tout repose sur la précision des gestes et la lisibilité des passages d'un corps à l'autre : le groupe doit gérer le focus pour que le public sache en permanence où se trouve la puce.", level: "Confirmé", objectives: ["Mime", "Écoute", "Corps", "Créativité", "Cohésion"], players: 0, duration: 10, energy: "Modérée", material: "Aucun", format: "En groupe simultané", phase: "Pré-impro", stageWarmup: true, publics: ["Enfants", "Ados", "Adultes", "Senior"] },
  { title: "Carrousel équestre", groupe: "Marche", summary: "Les joueurs déambulent en mimant qu'ils sont à cheval. Au bout d'un moment, ils s'alignent au centre du plateau. Puis ils redémarrent en cabrant leur monture et partent au galop. Ils passent au trot. Ils reviennent s'aligner. Et ainsi de suite, en laissant libre cours à leur imagination pour composer des chorégraphies équestres.", level: "Confirmé", objectives: ["Corps", "Cohésion", "Concentration", "Lâcher-prise", "Physique", "Créativité"], players: 0, duration: 15, energy: "Forte", material: "Aucun", format: "En groupe simultané", phase: "Échauffement", stageWarmup: true, publics: ["Enfants", "Ados", "Adultes"] },
  { title: "Les perroquets", groupe: "Vocal", summary: "En cercle, assez proches pour s'entendre sans forcer la voix. Le premier invente un son bref et le transmet à son voisin de droite. Celui-ci doit le répéter sous une forme interrogative, comme s'il demandait confirmation. Le premier écoute et indique par un signe clair, un hochement ou un geste convenu, si la reproduction lui convient. S'il approuve, le voisin invente à son tour un nouveau son qu'il transmet au suivant, qui l'imite de façon interrogative avant de recevoir l'approbation, et ainsi de suite jusqu'à ce que chacun ait proposé et transmis un son.", level: "Débutant", objectives: ["Voix", "Écoute", "Cercle", "Concentration", "Cohésion"], players: 0, duration: 15, energy: "Modérée", material: "Aucun", format: "En cercle", phase: "Échauffement", publics: ["Enfants", "Ados", "Adultes", "Senior"] },
  { title: "Trouver le meneur", groupe: "Cercle", summary: "En cercle, assis ou debout. Un joueur est désigné détective et sort de la pièce ou se place hors de vue. Pendant son absence, les autres choisissent discrètement un meneur. Quand le détective revient, tout le monde effectue les mêmes gestes : le meneur initie des mouvements simples, applaudir, taper du pied, lever les bras, tourner la tête, et les autres l'imitent le plus vite possible sans jamais attirer l'attention sur lui. Le détective observe le cercle et tente d'identifier qui est à l'origine des changements. Le meneur change régulièrement de geste pour compliquer la tâche. Quand le détective croit avoir trouvé, il désigne quelqu'un, puis un nouveau détective et un nouveau meneur sont choisis.", level: "Débutant", objectives: ["Observation", "Cercle", "Concentration", "Corps", "Cohésion", "Jeu"], players: 0, duration: 10, energy: "Modérée", material: "Aucun", format: "En cercle", phase: "Échauffement", warmup: true, publics: ["Enfants", "Ados", "Adultes", "Senior"] },
  { title: "Expressions imagées", groupe: "Clown", summary: "En cercle, debout. Chacun à son tour propose une expression imagée de la langue française qui fait référence à une partie du corps. L'expression est d'abord dite à voix haute, puis mimée ou illustrée corporellement. Par exemple : se serrer les coudes, prendre ses jambes à son cou, avoir la tête dans les nuages, en avoir plein le dos. Les autres écoutent, observent, et peuvent reprendre ou prolonger le geste proposé. L'intervenant veille à ce que chacun trouve une expression différente, jusqu'à explorer une grande variété de zones du corps et de formulations.", level: "Avancé", objectives: ["Mime", "Corps", "Créativité", "Voix", "Clown", "Cercle"], players: 0, duration: 15, energy: "Modérée", material: "Aucun", format: "En cercle", phase: "Pré-impro", publics: ["Ados", "Adultes"] },
  { title: "Les conseillers", groupe: "Comédie du jeu de la scène", summary: "Un personnage expose un problème ou demande un conseil. Trois autres interviennent chacun leur tour pour lui répondre. Le premier donne un bon conseil, le deuxième un mauvais conseil, et le troisième le pire conseil possible. Les prises de parole se succèdent dans cet ordre. Les personnages réagissent aux propositions et la conversation se poursuit autour des conseils formulés.", level: "Confirmé", objectives: ["Créativité", "Écoute", "Relation", "Personnages", "Jeu de la scène"], players: 0, duration: 15, energy: "Modérée", material: "Aucun", format: "Tour à tour avec spectateur", phase: "Impro", publics: ["Ados", "Adultes", "Senior"] },
  { title: "Alerte à la caserne", groupe: "Simplicité / jeu juste", summary: "Les personnages sont des pompiers dans une caserne, occupés à des actions ordinaires de leur quotidien. Soudain, une alerte retentit, clairement identifiable par un signal sonore ou un geste convenu. Immédiatement, ils interrompent ce qu'ils faisaient et passent en état d'intervention. Chacun enfile son équipement imaginaire, se prépare avec précision et se coordonne avec les autres pour partir. La scène se termine au moment du départ en intervention, quand tous sont prêts et engagés dans l'action.", level: "Confirmé", objectives: ["Mime", "Cohésion", "Rythme", "Personnages", "Lieux"], players: 0, duration: 15, energy: "Forte", material: "Aucun", format: "Tour à tour avec spectateur", phase: "Impro", publics: ["Ados", "Adultes"] },
  { title: "Pourquoi ?", groupe: "Comédie du jeu de la scène", summary: "Une improvisation est lancée avec deux personnages ou plus dans une situation simple donnée par l'intervenant. L'un d'eux adopte une règle précise : il répond ou réagit à chaque prise de parole des autres uniquement en demandant « pourquoi ? ». Systématiquement, quelles que soient les affirmations, propositions ou actions verbales des partenaires. Le ton, l'intention et la manière de le dire peuvent varier, mais la structure reste identique. Les autres doivent continuer l'improvisation sans sortir du jeu, en intégrant ces interruptions répétées.", level: "Confirmé", objectives: ["Jeu de la scène", "Écoute", "Émotions", "Personnages", "Rythme"], players: 0, duration: 15, energy: "Modérée", material: "Aucun", format: "Tour à tour avec spectateur", phase: "Impro", publics: ["Enfants", "Ados", "Adultes", "Senior"] },
  { title: "Jeu des 3 erreurs", groupe: "Observation, mimétisme et complicité", summary: "Les joueurs forment une image fixe représentant une toile de maître, comme une sculpture vivante. Chacun adopte une posture précise et la maintient sans bouger, de façon à composer une véritable scène figée. Un joueur, désigné comme le peintre, reste en retrait et observe attentivement l'ensemble, puis se retourne, dos au tableau. Pendant ce temps, un autre s'extrait discrètement de l'image en mémorisant exactement sa position, modifie trois détails dans la toile en changeant légèrement la gestuelle, la posture ou l'orientation de ses camarades, puis reprend sa place initiale. Le peintre se retourne et doit identifier les trois modifications, en les nommant une à une.", level: "Confirmé", objectives: ["Observation", "Mémoire", "Concentration", "Corps", "Cohésion"], players: 0, duration: 15, energy: "Faible", material: "Aucun", format: "En groupe simultané", phase: "Échauffement", publics: ["Ados", "Adultes"] },
  { title: "File d'attente muette", groupe: "Simplicité / jeu juste", summary: "Trois ou quatre personnages sont alignés les uns derrière les autres : ils font la queue. Le lieu n'est pas précisé, ce peut être une poste, un guichet, une boulangerie, un arrêt de bus, une administration. Chacun vit ce moment à sa manière : impatience, lassitude, rêverie, irritation, espoir, distraction. Ce sont les corps qui parlent : un soupir, un changement d'appui, un regard jeté sur l'autre, une petite avancée de quelques centimètres. Les réactions se répondent sans paroles et créent une mini-chorégraphie d'attente, où chaque micro-événement prend une valeur dramatique : quelqu'un avance, un autre s'énerve, un troisième s'endort.", level: "Confirmé", objectives: ["Jouer juste", "Observation", "Corps", "Silence", "Émotions", "Relation"], players: 0, duration: 15, energy: "Faible", material: "Aucun", format: "Tour à tour avec spectateur", phase: "Impro", publics: ["Ados", "Adultes", "Senior"] },
  { title: "Vérité immédiate", groupe: "Sentir ce qui manque", summary: "Deux joueurs ou plus improvisent une scène. Dès que l'un d'eux ressent une incohérence, une gêne ou un décalage, quelque chose qui cloche dans l'histoire, le ton ou la relation, il interrompt brièvement l'action sans quitter son rôle. Sans casser la scène, il dit à voix haute ce qu'il perçoit ou ressent à cet instant : une confusion, « je ne sais plus qui je suis ici » ; une émotion, « je me sens perdu » ; une observation, « quelque chose sonne faux entre nous ». Une fois cette vérité exprimée, la scène reprend aussitôt en tenant compte de ce qui vient d'être dit, comme si cette lucidité faisait partie du jeu. L'exercice peut se répéter plusieurs fois, chaque intervention servant à réajuster le fil en temps réel.", level: "Avancé", objectives: ["Écoute", "Jouer juste", "Émotions", "Relation", "Spontanéité"], players: 0, duration: 15, energy: "Faible", material: "Aucun", format: "Tour à tour avec spectateur", phase: "Impro", publics: ["Ados", "Adultes"] },
  { title: "Rugby réfractaire", groupe: "Comédie du jeu de la scène", summary: "Plusieurs joueurs entrent en scène et forment une mêlée, comme au rugby ou au football américain : formation compacte, corps contre corps, prêts à engager. Un ballon réel ou imaginaire est au centre. À chaque tentative d'envoi, l'un des personnages intervient pour interrompre le départ du jeu. Il trouve toujours un prétexte pour retarder, bloquer ou empêcher l'engagement. La mêlée se reforme sans cesse et l'envoi est systématiquement contrarié. La scène se développe sur cette tension entre l'élan collectif et le refus individuel.", level: "Avancé", objectives: ["Jeu de la scène", "Corps", "Créativité", "Cohésion", "Rythme"], players: 0, duration: 15, energy: "Forte", material: "Un ballon (facultatif)", format: "Tour à tour avec spectateur", phase: "Impro", publics: ["Ados", "Adultes"] },
  { title: "Chaises à l'infini", groupe: "Comédie du jeu de la scène", summary: "Les joueurs incarnent un groupe chargé de préparer un événement important. Leur tâche : installer des chaises sur le plateau, en suivant une logique précise ou symbolique. Mais à mesure que certains installent, d'autres, involontairement, déplacent, dérangent ou désinstallent ce qui vient d'être mis en place. Ce déséquilibre permanent crée un mouvement continu où l'ordre semble toujours sur le point d'être atteint sans jamais l'être. Aucune parole n'est nécessaire : tout passe par les gestes, les intentions, les regards et la coordination.", level: "Avancé", objectives: ["Cohésion", "Écoute", "Objet", "Silence", "Jeu de la scène", "Rythme"], players: 0, duration: 15, energy: "Modérée", material: "Des chaises", format: "En groupe simultané", phase: "Impro", publics: ["Adultes"] },
  { title: "Grands Woow et Bililipes", groupe: "Physique", summary: "Les joueurs sont répartis en deux groupes : un Grand Woow et des Bililipes. Le Grand Woow se déplace en effectuant de grands gestes lents et amples, chaque mouvement exagéré, étiré, au ralenti. Il cherche à toucher les Bililipes. Ceux-ci se déplacent comme des pingouins : petits pas rapides et serrés, bras le long du corps, légèrement écartés. Ils doivent éviter d'être touchés. Quand un Bililipe est touché, il se transforme immédiatement en Grand Woow, adopte les grands gestes lents et rejoint la traque. Le jeu continue jusqu'à ce que tous les Bililipes aient été transformés.", level: "Débutant", objectives: ["Corps", "Jeu", "Physique", "Lâcher-prise", "Rythme"], players: 0, duration: 10, energy: "Forte", material: "Aucun", format: "En groupe simultané", phase: "Échauffement", warmup: true, publics: ["Enfants", "Ados", "Adultes"] },
  { title: "Nudistes", groupe: "Clown", summary: "Les joueurs se placent dans l'espace scénique et miment le fait de se déshabiller entièrement, sans retirer aucun vêtement réel. Chaque geste est précis et progressif, comme s'ils enlevaient successivement différentes couches, jusqu'à ce que le déshabillage imaginaire soit terminé. Une fois nus, ils se mettent en mouvement et courent dans l'espace en se comportant comme s'ils l'étaient vraiment. Leur rapport à l'espace, aux autres et à leurs propres déplacements est entièrement guidé par cette situation imaginaire.", level: "Débutant", objectives: ["Corps", "Mime", "Lâcher-prise", "Confiance", "Clown"], players: 0, duration: 10, energy: "Forte", material: "Aucun", format: "En groupe simultané", phase: "Pré-impro", warmup: true, stageWarmup: true, publics: ["Adultes"] },
  { title: "Notre bon roi", groupe: "Les statuts", summary: "Un joueur incarne le roi ou la reine et s'assoit sur une chaise placée au centre de l'espace. Les autres viennent tour à tour devant lui pour lui faire des propositions destinées à le satisfaire : offrir un objet imaginaire, rendre un service, proposer une action, présenter un gâteau, offrir un divertissement. Le roi écoute et observe. À tout moment, il peut décider d'éliminer quelqu'un en claquant des doigts. L'éliminé quitte l'aire de jeu et un autre vient tenter sa chance.", level: "Débutant", objectives: ["Statut", "Créativité", "Mime", "Personnages", "Engagement"], players: 0, duration: 15, energy: "Modérée", material: "Une chaise", format: "Tour à tour avec spectateur", phase: "Pré-impro", publics: ["Enfants", "Ados", "Adultes", "Senior"] },
  { title: "Corde à sauter géante", groupe: "Physique", summary: "Deux joueurs se placent face à face et tiennent chacun une extrémité d'une longue corde, qu'ils font tourner régulièrement en la faisant passer près du sol, comme dans une cour d'école. Les autres se placent à proximité et entrent tour à tour au centre pour sauter en rythme, sans se prendre la corde dans les pieds. Ils peuvent rester seuls ou être plusieurs à l'intérieur en même temps, selon la consigne. La rotation est continue. Celui qui touche la corde ou se trompe de timing sort et échange sa place avec un tourneur, ou rejoint la file d'attente. On peut ensuite modifier la vitesse de rotation, le nombre de joueurs au centre ou la durée de passage.", level: "Confirmé", objectives: ["Rythme", "Corps", "Physique", "Concentration", "Cohésion", "Jeu"], players: 0, duration: 15, energy: "Forte", material: "Une longue corde", format: "Tour à tour avec spectateur", phase: "Échauffement", warmup: true, publics: ["Enfants", "Ados", "Adultes"] },
  { title: "Ennui au bureau", groupe: "Simplicité / jeu juste", summary: "Un personnage est installé devant une table sur laquelle se trouvent divers objets de bureau : stylos, cahier, sous-main, ordinateur. Son attitude et son état corporel établissent d'emblée qu'il est là depuis très longtemps. L'ennui est visible et s'inscrit dans la durée. Sans jamais quitter son espace, il cherche à s'occuper avec ce qu'il a sous la main : il détourne les objets, répète des actions, explore des micro-occupations, invente de petites activités. Toute la scène se construit à partir de cette situation statique, en laissant l'ennui guider le rythme et les propositions.", level: "Avancé", objectives: ["Jouer juste", "Objet", "Silence", "Rythme", "Personnages"], players: 0, duration: 15, energy: "Faible", material: "Une table et des objets de bureau", format: "Tour à tour avec spectateur", phase: "Impro", publics: ["Ados", "Adultes"] },
  { title: "Le seul chocolat", groupe: "Enjeux", summary: "Deux personnages entrent ensemble et découvrent une boîte posée dans l'espace. Ils l'ouvrent et constatent qu'elle ne contient qu'un seul chocolat. Ils prennent le temps d'observer la boîte, le chocolat, et la réaction de l'autre. L'improvisation se construit à partir de cette découverte unique : les hésitations, les tentatives d'appropriation, les négociations et les stratégies émergent librement. Ils restent engagés dans la situation jusqu'à ce qu'une décision soit prise.", level: "Débutant", objectives: ["Enjeux", "Objet", "Relation", "Regard", "Écoute"], players: 0, duration: 15, energy: "Faible", material: "Une boîte", format: "Tour à tour avec spectateur", phase: "Impro", publics: ["Enfants", "Ados", "Adultes", "Senior"] },
  { title: "5 minutes avant le meurtre", groupe: "Monter les enjeux", summary: "Une scène entre deux personnages, juste avant que l'un ne tue l'autre. Il s'agit de faire monter la tension et de donner au public des éléments de compréhension sur les motivations du meurtrier. Peut s'enchaîner directement avec l'exercice « Mort interminable ».", level: "Confirmé", objectives: ["Enjeux", "Émotions", "Relation", "Personnages", "Écoute"], players: 0, duration: 15, energy: "Modérée", material: "Aucun", format: "Tour à tour avec spectateur", phase: "Impro", publics: ["Ados", "Adultes", "Senior"] },
  { title: "Urgence à deux", groupe: "Monter les enjeux", summary: "Deux personnages entrent en scène dans un état d'urgence manifeste. Chacun est porté par une raison précise qui justifie son stress immédiat : un événement imminent, un problème à résoudre, une situation pressante, une contrainte extérieure. Ces raisons sont différentes pour chacun et ne sont pas connues de l'autre. Dès l'entrée, ils doivent agir et parler sous cette pression, en défendant coûte que coûte leur propre urgence, tout en restant à l'écoute de l'autre et en réagissant à ce qui se dit et se fait. L'échange se construit sur la coexistence de ces deux urgences, sans que l'une ne prenne totalement le dessus.", level: "Confirmé", objectives: ["Enjeux", "Émotions", "Écoute", "Engagement", "Relation"], players: 0, duration: 15, energy: "Forte", material: "Aucun", format: "Tour à tour avec spectateur", phase: "Impro", publics: ["Ados", "Adultes"] },
  { title: "Cadeau nul adoré", groupe: "Acceptation et écoute", summary: "Deux personnages se font face. Le premier offre un cadeau imaginaire en le présentant clairement et en annonçant ce que c'est. Il doit inventer le cadeau le plus nul, le plus inutile ou le plus décevant possible : une chaussette trouée, une vieille boîte vide, un chewing-gum mâché, un caillou sans intérêt. Pourtant, le second, contre toute attente, se montre fou de joie. Il s'émerveille, remercie, s'enthousiasme et développe avec conviction toutes les raisons pour lesquelles ce cadeau est extraordinaire, unique, émouvant ou précieux.", level: "Confirmé", objectives: ["Acceptation", "Émotions", "Créativité", "Écoute", "Objet"], players: 0, duration: 15, energy: "Modérée", material: "Aucun", format: "Tour à tour avec spectateur", phase: "Impro", publics: ["Ados", "Adultes", "Senior"] },
  { title: "Dessus-dessous", groupe: "Les statuts", summary: "Deux joueurs improvisent un dialogue. Leurs positions physiques sur le plateau traduisent en permanence le rapport de force entre leurs personnages. Celui qui détient momentanément l'avantage est debout, mobile, affirmé. L'autre, en position de faiblesse ou d'écoute, adopte une posture plus basse : assis, accroupi, à genoux, ou même au sol. Au fil de l'échange, ces statuts évoluent : un mot, un ton, un silence ou un geste peuvent inverser la hiérarchie, et les joueurs doivent ajuster leurs positions instantanément.", level: "Avancé", objectives: ["Statut", "Corps", "Relation", "Écoute", "Posture"], players: 0, duration: 15, energy: "Modérée", material: "Aucun", format: "Tour à tour avec spectateur", phase: "Impro", publics: ["Adultes"] },
  { title: "Pilotes farceurs", groupe: "Comédie du jeu de la scène", summary: "Deux personnages sont installés dans le cockpit d'un avion en vol, aux commandes, avec des passagers à bord. Ils prennent régulièrement la parole au micro destiné aux annonces de bord. À travers ces annonces, ils font volontairement des blagues ambiguës, inquiétantes ou décalées, laissant entendre des situations potentiellement alarmantes ou absurdes. Ils se répondent, se surenchérissent, s'amusent entre eux, en imaginant la réaction des passagers. Tout le jeu repose sur le contraste entre le ton officiel attendu d'une annonce et le contenu volontairement perturbant des messages diffusés.", level: "Avancé", objectives: ["Voix", "Complicité", "Jeu de la scène", "Créativité", "Personnages"], players: 0, duration: 15, energy: "Modérée", material: "Aucun", format: "Tour à tour avec spectateur", phase: "Impro", publics: ["Ados", "Adultes"] },
  { title: "Douleur en silence", groupe: "Clown", summary: "Le joueur incarne un personnage qui se cogne accidentellement une partie du corps : la tête, le genou, le coude. Il ne s'agit pas d'un vrai choc, mais d'un mime précis et crédible du moment de l'impact. Après ce faux coup, il exprime silencieusement sa douleur en exagérant largement ses réactions. Il amplifie chaque ressenti par le corps, les gestes, les mimiques et le souffle, sans jamais prononcer un mot. Ça peut aller jusqu'à l'excès, comme dans une mort interminable burlesque. On peut rejouer plusieurs fois la même situation en variant l'intensité, la durée ou la localisation du choc.", level: "Confirmé", objectives: ["Clown", "Corps", "Émotions", "Silence", "Rythme", "Mime"], players: 0, duration: 15, energy: "Modérée", material: "Aucun", format: "Tour à tour avec spectateur", phase: "Pré-impro", stageWarmup: true, publics: ["Ados", "Adultes"] },
].map((e) => ({ id: uid(), warmup: false, stageWarmup: false, application: false, favorite: false, showTypes: [], thematiques: [], createdByUser: false, ...e }));
...
function mergeMissingCategories(data) {
  ...
  const exercisesToAdd = [...EXERCICES_ECHAUFFEMENT, ...EXERCICES_PRE_IMPRO, ...EXERCICES_IMPRO, ...EXERCICES_NOUVEAUX_2026].filter((e) => !existingExerciseTitles.has(e.title));
...
function buildCours(exercises, categories, { niveau, tempsTotal, nbEchauffements, nbExercices, nbImpro, objectifs, thematiques, participants, joueursSeConnaissent, faireAmbassadeur, recentIds, integrerFavoris }) {
  ...
  const used = new Set();
  // Dernier échauffement retenu jusqu'ici, en tenant compte de ceux qu'une passe en cours vient
  // d'ajouter mais qui ne sont pas encore recollés à `warmups` : sert à ne pas enchaîner deux
  // fiches qui se recoupent (voir EXERCICES_PROCHES).
  const dernierEchauffement = (picked = []) => picked[picked.length - 1] || warmups[warmups.length - 1];
  const fillFromPool = (pool, remaining, maxCount = Infinity) => {
    ...
    for (const e of shuffled) {
      ...
      if (sontProches(e, dernierEchauffement(picked))) continue;
  ...
  const pickFamilyWarmup = (families) => {
    ...
    const retenu = sansLesProchesDe(prioritized.length > 0 ? prioritized : pool, dernierEchauffement());
    const pick = pickRandom(prioritizeFresh(retenu));
  ...
  const pickFromPool = (pool) => {
    ...
    // Jamais deux fiches qui se recoupent l'une derrière l'autre (EXERCICES_PROCHES) — la carte qui
    // précède peut être le dernier échauffement, puisque sur le plan imprimé elles se suivent.
    choicePool = sansLesProchesDe(choicePool, middle[middle.length - 1] || warmups[warmups.length - 1]);
  ...
  const generate = () => {
    ...
    const fillFrom = (pool) => {
      ...
      for (const e of shuffled) {
        ...
        // Même règle que dans le générateur de cours : deux fiches qui se recoupent ne s'enchaînent
        // pas (EXERCICES_PROCHES). Le bouton "Aléatoire" d'une carte, lui, reste libre.
        if (sontProches(e, picked[picked.length - 1])) continue;
